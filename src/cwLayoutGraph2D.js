    ...
    for (var associationNode in child.associations) {
      if (child.associations.hasOwnProperty(associationNode)) {
        ...
        for (var i = 0; i < child.associations[associationNode].length; i += 1) {
          ...
          if (!this.configuration.mesurementNodes.hasOwnProperty(associationNode)) {
            ...
            config.propertyYScriptname.forEach(function(property) {
              ...
              if (propertType.type === "Lookup") {
                y = nextChild.properties[property + "_abbreviation"];
              }

              if (config.name) {
                group = config.name;
              } else {
                group = propertType.name;
              }
